function[theta, depthDay, depthNight, avlocDay, avlocNight] = calcpreference(param,bottom,photicm)
   ...
   xrange = linspace(0,bottom,(bottom+1));
   dvm = photicm + 500; % vertical migration depth
   dvm(bottom < (photicm + 500)) = bottom; % migration to bottom in intermediate habitats
   dvm(bottom <= param.mesop) = 0; % no migration in shallow habitats
   ...
   % demersal fish day
   demmig = dvm;
   demmig((bottom - dvm) >= 1200) = dvm + (bottom-dvm-1200);
   demmig((bottom - dvm) >= 1500) = bottom;
   ...
   [~,idi]  = maxk(depthDay,2);
   avlocDay = xrange(idi);
   avlocDay(2,[1 2 pelpred]) = dvm; % capture bimodal peak
   avlocDay = mean(avlocDay,1);
       
   %
   % calculate center of vertical distribution during the night
   [~,idi] = maxk(depthNight,2);
   avlocNight = xrange(idi);
   avlocNight = mean(avlocNight,1);
